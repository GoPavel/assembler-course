...
==NB)== некоторые компиляторы, например yasm, позволяют в скобках писать 5*eax. Они это просто сконвертируют, к верхней схеме, если смогут.
...
==NB)== Кто выделяет память под возвращаемое значение, если оно не лезет? Только вызывающий, так как память нужно освобождать ровно так, как выделялась.

==NB)== Сохранять все регистры, также как и не сохранять ничего -- дорого.
...
==NB)== Эта команда сильно похожа на `finit`, но последнаяя также устанавливает дефолтное значение.
...
- Любая программа имеет доступ ко всему. То есть обращение к памяти нельзя ограничить.
- Двойственность: к одной памяти можно обратиться из разных сегментов и это сложно котролировать.

### Protected mode 16

Начиная с 286 было выделено 4 кольца прав.
- 0 - макс. права
- 3 - мин. права
...
- g/l -- global/local description table, но локальные не используется.
    - gdtr -- регистр, который указывает на адрес таблицы декрипторов.
- rpl -- права
...
CPL(current privilege level) - кольцо прав в cs. Оно определяет права процесса.
...
**NB)** Не путать сегмент с секцией. Секция -- это уровень операционной системы. Сегмент -- это уровень процессора.
...
В 32bit у нас Flat-module памяти. Все базы сегментов равны 0 и имеют максимальный лимит 4Gb.
