...
В курсе рассматривается только архитектура x86, потому что она очень распространенная среди PC и появилась довольно давно, что позволяет ожидать хорошую совместимость программ написанных на ассемблере.

Сначала мы рассмотрим x86, потом x64 (x86-64 bit)
...
- FMA3, FMA4 (2010) : fased multiply-add, инструкции типа `a*b + c` для `x\ymm` 
...
## Режимы работы процессора (32bit)
...
**Проверить:** Видимо лимит на размер сегмента в реальном моде даже на i80186 хранился в скрытой части сегмента(видимо тогда она уже сущетсвовала.). Но я не нашел явно упоминания.
...
**System Management Mode** - режим отладки **TODO**
...
#### Почему на некоторых Win32 нельзя выделить больше 4 ГБ?
...
**Замечание** Это очень гибкий механизм, но долгий. Поэтому сущетсвует TLB(translation lookaside buffer)-кеш.
...
- PSE: добавляется возможность создавать большие страницы по 4Mb. Для их меняется PTE, но не PDE, поэтому можно использовать и маленькие, и большие страницы.
- PAE: Мы делаем структуру, которая описывает страницу жирнее не 32 бита а 64, чтобы адресовать больше адресов. Из-за этого меньше page table, поэтому есть еще один уровень. `[31..30, 29..21, 20..12, 11..0] = [2, 9, 9, 12]`
...
**Замечание:** Лучше всего если ваша программа будет иметь весь код в одной битности, чтобы не было переключений. И вообще там не все так просто...
...
- Расширение общих регистров до 64 бит. (`rax`, `rbx` ...) 32битные названия - эти младшие части.
...
```asm
	mov al, bh % ok
	mov r9b, gh % нельзя, так как коды таких команды отданы под sil, dil
```
...
- выражения в скобках `[base_register + index_register * scale_factor + off32]`
  ...
  - `scale_factor` - 1, 2, 4, 8
  - `off32` - это знаковая константа. И ее битность являет проблемой. ТУДУ
- Можно использовать `rip` для указания смещения по коду. ТУДУ
- **Исключения**
...
- во всех конвенция теперь обязательно выравнивать стек до 16.
...
Она заключается в том на стеке всегда выделяется 256 байт вне зависимости от количества аргументов. Оно нужно для того, чтобы скинуть все аргументы на стек.
...
Она заключается в том, что мы можем писать на 128 ниже `rsp`.
